  ...
  return Object.keys(packagesSummaryPaths).reduce<ConsolidatedCoverageReport>(
    (summary, packageName) => {
      ...
      if (fs.existsSync(reportPath)) {
        const report: { total: CoverageSummary } = JSON.parse(fs.readFileSync(reportPath, 'utf8'));
...
function createCoverageReportForVisualRepresentation(
  ...
  return Object.keys(coverageReport).reduce<Record<string, Record<string, number>>>(
    (report, packageName) => {
